...
def post_to_pastebin(pokemon_name, pokemon_info):
    ...
    resp_mesg = requests.post(PASTEBIN_API_POST_URL, data=post_params)
    
    if resp_mesg.status_code == requests.codes.ok:
        print("Success!")
        return resp_mesg.text.strip()  # Assuming that the response contains just the paste URL
    else:
        print("Failure")
        print(f'Response code: {resp_mesg.status_code} ({resp_mesg.reason})')
        return None
